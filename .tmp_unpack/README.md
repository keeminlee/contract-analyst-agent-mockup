...
### Mock UI Report (Evidence-First Contract View)

Generate a stakeholder-facing HTML report from Bronze + Evidence artifacts:
...
- `demo_reports/credit_precision_tree.html` (tree-only mode)

Direct render command:

```bash
python scripts/render_mock_ui.py --bronze docs/bronze/sample_credit_agreement.bronze.json --evidence docs/cache/demo_outputs/credit_precision.evidence_packet.json --out demo_reports/credit_precision_mock_ui.html --print
```

Tree-only export for slides:

```bash
python scripts/generate_mock_report.py --scenario credit_precision --tree-only
```

For deterministic hash checks, add `--no-timestamp`.
